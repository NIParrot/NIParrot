...
class NI_Api_route{
    ...
    public static function put($action, Closure $callback){
        $routepath = '/'.implode('',explode('/',explode("?", $action)[0]));
        if (!key_exists($routepath,self::$PutRoutes)) self::$PutRoutes[$routepath] = $callback;
    }

    public static function delete($action, Closure $callback){
        $routepath = '/'.implode('',explode('/',explode("?", $action)[0]));
        if (!key_exists($routepath,self::$DeleteRoutes)) self::$DeleteRoutes[$routepath] = $callback;
    }
