...
function timeout(PromiseInterface $promise, $time, LoopInterface $loop)
{
    ...
    return new Promise(
        function ($resolve, $reject) use ($loop, $time, $promise) {
            $timer = null;
            ...
            $timer = $loop->addTimer(
                $time, function () use ($time, &$promise, $reject) {
...
{
    return new Promise(
        function ($resolve) use ($loop, $time, &$timer) {
            // resolve the promise when the timer fires in $time seconds
            $timer = $loop->addTimer(
                $time, function () use ($time, $resolve) {
                    $resolve($time);
                }
            );
